...
for enum, path in enumerate(input_folders):
    ...
    for train_index, test_index in kf.split(X):
        ...
        for i in train_index:
            cv2.imwrite(output_path + "/Fold{}/Train/{}/{}".format(nFold, categories[enum], names[i]), X[i])
        for i in test_index:
            cv2.imwrite(output_path + "/Fold{}/Validation/{}/{}".format(nFold, categories[enum], names[i]), X[i])
        nFold += 1
